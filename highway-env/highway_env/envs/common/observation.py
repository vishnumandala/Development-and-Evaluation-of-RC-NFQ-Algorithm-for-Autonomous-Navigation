...
class KinematicObservation(ObservationType):
    ...
    def observe(self) -> np.ndarray:
        ...
        if close_vehicles:
            ...
            additional_vehicles_df = pd.DataFrame.from_records(
                [v.to_dict(origin, observe_intentions=self.observe_intentions)
                for v in close_vehicles[-self.vehicles_count + 1:]]
            )[self.features]
            df = pd.concat([df, additional_vehicles_df], ignore_index=True)
        # Normalize
        if self.normalize:
        ...
        if df.shape[0] < self.vehicles_count:
            ...
            additional_vehicles_df = pd.DataFrame(data=rows, columns=self.features)
            df = pd.concat([df, additional_vehicles_df], ignore_index=True)
